...
var spinner = ora({
  ...
  for (const campaign of unactive_campaign) {
    ...
    if (!fs.existsSync(`./List_Archive/${campaign_id}`)){
        spinner = ora({
            text : `Creating Folder for ${campaign_id}` ,
            color : 'blue'
        })
        spinner.start()
      ...
      setTimeout(()=>{
        spinner.stopAndPersist({
          symbol : '✔'.green ,
          text: ` ${campaign_id} Folder created with success ! `.green.bold
        });
   ...
   spinner = ora({
       text : `Start downloading ${campaign_id}'s lists` ,
       color : 'blue'
   })
    spinner.start()
    ...
    if(list_ids_element.active == 'N'){
      ...
      spinner2 = ora({
          text : `Downloading ${list_name[0].list_name} \n ` ,
          color : 'blue'
      })
      spinner2.start()
